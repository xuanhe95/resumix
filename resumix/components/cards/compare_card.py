...
T = LANGUAGES[st.session_state.lang if st.session_state.lang else "en"]
...
def compare_resume_sections(
    ...
    for section_name, section_obj in sections.items():
        ...
        if use_card_template:
            ...
            _display_comparison_card(section_name, section_obj)
        else:
            # Original two-column version
            _display_comparison_columns(section_name, section_obj)


def _display_comparison_card(section_name: str, section_obj: SectionBase):
    ...
    display_card(
        ...
        additional_content=_generate_comparison_content(section_name, section_obj),
        dimensions=["相似度", "专业性", "匹配度", "完成度", "数据支撑"],
    )


def _display_comparison_columns(section_name: str, section_obj: SectionBase):
...
def _generate_comparison_content(section_name: str, section_obj: SectionBase) -> str:
